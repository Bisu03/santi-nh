...
const customer = () => {
  ...
  }, [searchQuery, medicineCharges, setAddType, setTotal, customer]);

  useEffect(() => {
    const fetchDetailsByAdmissionID = async () => {
      const { data } = await axios.get(`/api/customerbill/${customer}`);
    ...
    fetchDetailsByAdmissionID();
  }, [setCart, setMedicineCharges, setTotal, customer]);
  ...
  const handleSubmit = async () => {
    ...
    if (isMedAdded === true) {
      try {
        await axios.put("/api/updatecustomermedicine", {
          id: isMedAddedId,
          medicineCharges,
          total,
        });
        setMedSave(false);
        router.push(`/customerbill/${customer}`);
      } catch (error) {
        setMedSave(false);
      ...
      try {
        await axios.post("/api/addcustomermed", {
          customerId: customer,
          medicineCharges,
          total,
        });
  ...
  return (
    <>
      <NavbarTwo />
      <Heading title="Add Medicine To Customer" />
      <table className=" container table table-compact w-full my-4 ">
        <thead>
          <tr>
            <th>Unique Id- {admissionData?.uniqueId}</th>
            <th>Name- {admissionData?.fullname}</th>
            <th>Gender- {admissionData?.gender}</th>
      ...
      <div className="flex justify-center w-full my-4 ">
        ...
        <Link href="/customerrecord" className="btn btn-info">
          Cancel
        </Link>
      </div>
    </>
